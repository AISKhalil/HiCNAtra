function obj = OneDplusCnNormalization(obj)
...
%
preNormFilePath = strcat(dir, '/', 'contactMapPreNorm_OneDplusCn',int2str(obj.contactMapBinSize),'.xls');
postNormFilePath = strcat(dir, '/', 'contactMapPostNorm_OneDplusCn',int2str(obj.contactMapBinSize),'.xls');
%
...
%%%%%%%%
	if (length(f) > 1)
		...
		%% 2) correction
		fNorm1 = fMat ./sqrt(yGenome);
		fNorm  = fNorm1 ./sqrt(cMat);
	else
		fNorm = [];
	end
	%%%
