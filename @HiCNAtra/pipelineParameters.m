...
%Estimating the HiCNAtra thresholds based on data coverage using exponential regression model.
...
d = -0.008899;
x = dataCoverage;
minRegionWidthForTestInBins = ceil(obj.minimumFAsize/obj.binSize);
...
d2 = -0.02144;
x2 = dataCoverage;
obj.amplificationThreshold = min(max(round(a2*exp(b2*x2) + c2*exp(d2*x2),3), 0.9),1);
...
d3 = -0.01981;
x3 = dataCoverage;
obj.deletionThreshold = min(max(round(a3*exp(b3*x3) + c3*exp(d3*x3),3), 0.9),1);
