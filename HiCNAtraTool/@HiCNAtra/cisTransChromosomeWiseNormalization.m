function obj = cisTransChromosomeWiseNormalization(obj)
...
for i  = 1:1:noChrs 
	...
	for j = i:1:lastChr
		...
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		%%-------- Normalization --------%%		
		[f, m, g, e, c, nRow, nCol, nElement, filteredIndices] = normalizationFeatures(obj, filteredRawIntMatrix, mapp1Track, mapp2Track, GC1Track, GC2Track, effLen1Track, effLen2Track, CNVs1Tracks, CNVs2Tracks);
		[fNorm] = genomeGLMDataNormalization(f, m, g, e, c);
		%%%
		if(~isempty(fNorm))
		...
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		[cCorr,~]  = corr(f, c,'Type','Spearman');
		[eCorr,~]  = corr(f, e,'Type','Spearman');
		[gCorr,~]  = corr(f, g,'Type','Spearman');		
		[mCorr,~]  = corr(f, m,'Type','Spearman');
		preSpearmanCorrelation = [cCorr, eCorr, gCorr, mCorr];
		preNormArray = [preNormArray; preSpearmanCorrelation];
		%%
		[cCorr,~]  = corr(fNorm, c,'Type','Spearman');
		[eCorr,~]  = corr(fNorm, e,'Type','Spearman');
		[gCorr,~]  = corr(fNorm, g,'Type','Spearman');		
		[mCorr,~]  = corr(fNorm, m,'Type','Spearman');
...
preNormFilePath = strcat(dir, '/', 'contactMapPreNorm_chrWise',int2str(obj.contactMapBinSize),'.xls');
postNormFilePath = strcat(dir, '/', 'contactMapPostNorm_chrWise',int2str(obj.contactMapBinSize),'.xls');
%
A = preNormArray;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [fNorm] = genomeGLMDataNormalization(f, m, g, e, c, toolID)
%%%%%%%%
	if (length(f) > 1)
