function obj = readsExtractionRestrictionFragmentsMid(obj)
...
for i  = 1:1:noChrs 
    ...
    %%
    chrRfragMid   = (chrRfragStart + chrRfragEnd)/2;
    midRfragBins = floor(chrRfragMid/binSize) + 1;
    %%
    noReads = length(midRfragBins);
    for j = 1:noReads
        cutBin = midRfragBins(j);
        readCountsBins(cutBin) = readCountsBins(cutBin) + chrRfragCount(j);
    end
    %%
